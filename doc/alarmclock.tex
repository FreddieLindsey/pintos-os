...
Changes made to \texttt{struct thread} are shown above. We used a 64-bit integer to store the time left until the thread should be unblocked and run. We had chosen a signed integer to ensure that if a negative value was given, the thread would awaken again immediately rather than the value overflowing and the thread sleeping for a long time. This was a vital component of ridding busy waiting. Also, we used a semaphore per thread to give a higher level construct to manipulate thread (un)blocking.
