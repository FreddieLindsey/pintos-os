...
Each thread's \texttt{struct} contains a semaphore. In \texttt{timer$\_$sleep()}, the function \texttt{sema$\_$down()} is called on the current thread. This then blocks the current thread.
