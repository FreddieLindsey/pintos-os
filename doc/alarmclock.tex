...
Changes made to \texttt{struct thread} are shown above. We used a 64-bit integer to store the time left until the thread should be unblocked and run. A signed integer was chosen that was in the past - should a thread already have been awoken at the point of execution. This was a vital component of ridding busy waiting. Also, we used a semaphore per thread to give a higher level construct to manipulate thread (un)blocking.
